...
def proj_mat_inverse(theta, X, Y):
    X = np.array(X); Y = np.array(Y);theta = np.array(theta)
...
def proj_mat(R):
    ...
    R_inversed[:, 0, 2] = -R_inversed[:,0,0]*R[:,0,2] -R_inversed[:,0,1]*R[:,1,2]
    R_inversed[:, 1, 2] = -R_inversed[:,1,0]*R[:,0,2] -R_inversed[:,1,1]*R[:,1,2]
    return R_inversed
