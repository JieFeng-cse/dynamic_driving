...
from LSTM_model import graph_attention, new_model
...
def evaluate_fourier(dataset_loader, model, criterion, criterion2, optim, batch_size, device, epoch_id):
    ...
    for Xs, Labels, R_inversed in dataset_loader:
        ...
        v0 = torch.mul(Labels[:, 0, 2], torch.cos(psi)) + \
        ...
        Label_world = torch.matmul(R_inversed.unsqueeze(1).repeat(1, 30, 1, 1), torch.cat([(Labels[:,:,0:2]).unsqueeze(
            2), torch.ones([Labels.shape[0], Labels.shape[1], 1, 1]).to(device).double()], dim=3).transpose(2, 3))
...
#         Xs = Xs.double().to(device)
#         Labels = Labels.double().to(device)
#         t = torch.arange(start=0, end=40, step=1).unsqueeze(
#             0).repeat(Labels.shape[0], 1)
#         t = t.double().to(device)
#         t.requires_grad = True
#         psi = Labels[:, 0, 4].double().to(device)
...
#         traj_world = torch.matmul(R_inversed.unsqueeze(1).repeat(1, 40, 1, 1), torch.cat([(output).unsqueeze(
#             2), torch.ones([output.shape[0], output.shape[1], 1, 1]).to(device).double()], dim=3).transpose(2, 3))
#         traj_world = traj_world.squeeze()[:, :, 0:2]
#         # print(traj_world.shape)

#         Label_world = torch.matmul(R_inversed.unsqueeze(1).repeat(1, 40, 1, 1), torch.cat([(pos_transed).unsqueeze(
#             2), torch.ones([pos_transed.shape[0], pos_transed.shape[1], 1, 1]).to(device).double()], dim=3).transpose(2, 3))
#         Label_world = Label_world.squeeze()[:, :, 0:2]

#         if i % 100 == 0:

#             print("start prediction: ", output[0][0], " pos: ", pos_transed[0][0])
#             print("end prediction: ", output[0][-1], " pos: ", pos_transed[0][-1])
...
def evaluate(validation_loader, model, criterion, batch_size, device, epoch_id):
    ...
    with torch.no_grad():
        for Xs, Labels in validation_loader:
            ...
            if i % 200 == 0:
                print('first: ground truth:', Labels[0], 'prediction:', output[0])
                print('last: ground truth:', Labels[-1], 'prediction:', output[-1])
                # L = np.mean((Labels.cpu().numpy() - output.cpu().numpy())**2)
                # print(L, criterion(output, Labels))
                R_inversed = proj_mat_inverse(R.squeeze(), device)
                traj_world = torch.matmul(R_inversed.unsqueeze(1).repeat(1, 30, 1, 1), torch.cat([out_tmp[:, :, 0:2].unsqueeze(
                    2), torch.ones([out_tmp.shape[0], out_tmp.shape[1], 1, 1]).to(device).double()], dim=3).transpose(2, 3))
                traj_world = traj_world.squeeze()[:, :, 0:2]
                main_drawer('/home/jonathon/Documents/new_project/interaction-dataset-master/maps/DR_CHN_Merging_ZS.osm',
                            label_tmp[0].cpu().detach().numpy(), traj_world[0].cpu().detach().numpy(), epoch_id)
...
def train(dataset_loader, model, criterion, optim, batch_size, device):
    ...
    for Xs, Labels in dataset_loader:
        # model.zero_grad()
        # Xs = Xs[:,:,:,1:5] #batchsize * seqlen * node_num * dim
        Xs = Xs.double().to(device)
        state = 'train'
        Labels = Labels.double().to(device)
        R, output = model(Xs, state)
...
def train_fourier(dataset_loader, model, criterion, criterion2, optim, batch_size, device, epoch_id):
    model.train()
    total_loss_xy = 0
    total_loss_vxy = 0
    n_samples = 0
...
parser.add_argument('--val_batch_size', type=int,
                    default=512, metavar='N', help='batch size')
parser.add_argument('--gpu', type=int, default=0)
parser.add_argument('--mode', type=str, default='train', help='train or test')
parser.add_argument('--log_interval', type=int, default=2000,
                    metavar='N', help='report interval')

parser.add_argument('--save', type=str,  default='/home/jonathon/Documents/new_project/interaction-dataset-master/model_logs/',
                    help='path to save the final model')
parser.add_argument('--cuda', type=str, default=True)
parser.add_argument('--optim', type=str, default='adam')
parser.add_argument('--gc', type=str, default='icra',
                    help='type of graph constructor')  # 'DynamicMts'
...
parser.add_argument('--fusion', type=bool, default=True,
                    help='fuse information from different frames before or after gc')
parser.add_argument('--multigraph', type=bool,
                    default=True, help='multipul graphs')
...
parser.add_argument('--frame_n', type=int, default=40,
                    help='every case have n frames')
parser.add_argument('--frame_gap', type=int, default=10,
                    help='frame gap between two agents')
parser.add_argument('--normalization', type=bool, default=True,
                    help='whether to do batch normalization')
parser.add_argument('--model', type=str, default='fourier', help='Model name')
...
if args.model == 'icra':
...
elif args.model == 'rnn' or args.model == 'rnn_update':
    ...
    feature_map = vectors_car_road(
        '/home/jonathon/Documents/new_project/interaction-dataset-master/maps/DR_CHN_Merging_ZS.osm', npy_path, 10, 30)
    val_map = vectors_car_road(
        '/home/jonathon/Documents/new_project/interaction-dataset-master/maps/DR_CHN_Merging_ZS.osm', val_path, 10, 30)
if args.gpu != None:
...
criterion2 = nn.SmoothL1Loss().cuda()
optim = Optim.Optim(
...
try:
    if args.mode == 'train':
        ...
        for epoch in range(1, args.epochs+1):
            ...
            if args.model == 'rnn' or args.model == 'rnn_update':
                train_loss = train(dataset_loader, model,
                                   criterion, optim, args.batch_size, device)
                # val_loss = 0
                val_loss = evaluate(
                    val_loader, model, criterion, args.val_batch_size, device, str(epoch)+args.model)
                print('| end of epoch {:3d} | time: {:5.2f}s | xy_loss {:5.9f} | val_loss {:5.9f} |'.format(
                    epoch, (time.time()-epoch_start_time), math.sqrt(train_loss), math.sqrt(val_loss)))
            elif args.model == 'fourier':
                xy_loss, v_loss = train_fourier(
                    dataset_loader, model, criterion, criterion2, optim, args.batch_size, device, str(epoch)+args.model)
        ...
        epoch_start_time = time.time()
        test_loss = evaluate(val_loader, test_model,
                             criterion, args.batch_size)
        print('| time: {:5.2f}s | test_loss {:5.9f} |'.format(
            (time.time()-epoch_start_time), math.sqrt(test_loss*1000000)))
