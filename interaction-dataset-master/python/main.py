...
def evaluate(validation_loader,model,criterion,batch_size):
    model.eval()
    total_loss = 0
    n_smaples = 0
    i = 0
...
def train(dataset_loader,model,criterion,optim, batch_size):
    model.train()
    total_loss = 0
    n_smaples = 0
...
try:
    if args.mode == 'train':
        ...
        for epoch in range(1, args.epochs+1):
            ...
            train_loss = train(dataset_loader,model,criterion,optim,args.batch_size)
            val_loss = evaluate(val_loader,model,criterion,args.batch_size)
