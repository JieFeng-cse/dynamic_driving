...
def evaluate_fourier(dataset_loader,model,criterion,optim, batch_size, device, epoch_id):
    model.eval()
    total_loss_xy = 0
    total_loss_vxy = 0
    n_samples = 0
    ...
    if args.mode == 'train':
        ...
        for epoch in range(1, args.epochs+1):
            ...
            if args.model == 'rnn' or args.model == 'rnn_update':
                ...
                # val_loss = 0
                val_loss = evaluate(val_loader,model,criterion,args.val_batch_size, device, str(epoch)+args.model)
                ...
                xy_loss_val,v_loss_val = evaluate_fourier(val_loader,model,criterion,optim, args.val_batch_size, device, str(epoch)+args.model)
                print('Train: | end of epoch {:3d} | time: {:5.2f}s | xy_loss {:5.9f} | vel_loss {:5.9f} |'.format(epoch,(time.time()-epoch_start_time),math.sqrt(xy_loss),math.sqrt(v_loss)))
                print('Val: | end of epoch {:3d} | time: {:5.2f}s | xy_loss_val {:5.9f} | vel_loss_val {:5.9f} |'.format(epoch,(time.time()-epoch_start_time),math.sqrt(xy_loss_val),math.sqrt(v_loss_val)))
